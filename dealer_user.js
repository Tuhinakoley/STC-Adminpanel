...
function showUserList(){
	...
	if(user_details.user_role_name == "Admin"){	
				...
				success: function(data){
					if(data.error){
						...
						$.map(data.user_list,function(item){
							...
							//table.row.add([item.first_name+' '+item.last_name, item.mobile_number, item.email_id, item.city_name, item.parent_first_name+' '+item.parent_last_name, status,editButton,'<button type="button" class="btn btn-info" data-tooltip="Edit" title="Edit" id="edit_member'+i+'" onclick="updateMemberDetails(\''+item.id+'\');"><i class="fa fa-edit" aria-hidden="true" style="zoom: 1.5;"></i></button>']).draw();
							table.row.add(['<a style="cursor: pointer;color: blue;" onclick="showUserDetails('+item.id+');">'+item.first_name1+' '+item.last_name+'</a>', item.mobile_number, item.email_id, item.city_name,item.parent_first_name+' '+item.parent_last_name, status,editButton,'<button type="button" class="btn btn-info" data-tooltip="Edit" title="Edit" id="edit_member'+i+'" onclick="showeditUserDetails(\''+item.id+'\');"><i class="fa fa-edit" aria-hidden="true" style="zoom: 1.5;"></i></button>']).draw();
		...
		$.ajax({
				url: serverURL + 'get_dealer_details_by_distributer_id',
				dataType: 'json',
				type: 'post',
				data:{
	...
	$.ajax({
				...
				success: function(data){
					if(data.error){
						...
						var parent_id = data.parent_user_id;
						...
						if(data.company_logo !=""){
							...
							document.getElementById("editcompany_pic1").src = imgURL+data.company_logo;
						...
						if(user_details.user_role_name == "Admin"){
							$.ajax({
									...
									success: function(data){
										if(data.error){
											$("#edit_distributer").html('');
											$("#edit_distributer").append('<option value="">Select Distributer</option>');
										}else{
											$("#edit_distributer").html('');
											...
											$.map(data.user_list,function(item){
												...
												if(item.id == parent_id){
													...
													$("#edit_distributer").append('<option value="'+item.id+'" selected>'+item.first_name1+' '+item.last_name+'</option>');
												}else{
													$("#edit_distributer").append('<option value="'+item.id+'">'+item.first_name1+' '+item.last_name+'</option>');
...
function openModal(){
	...
	$("#add_distributer").html('');
	$("#add_distributer").append('<option value="">Select Distributer</option>');
...
function addUserDetails(){
	...
	if($("#add_first_name").val()!="" && $("#add_last_name").val()!="" && $("#add_email").val()!="" && $("#add_mobile").val()!="" && $("#add_password").val()!="" && $("#add_confirm_password").val()!="" && $("#add_city").val()!="" && $("#add_state").val()!="" && $("#add_pincode").val()!="" && $("#add_distributer").val()!="" && $("#add_profile_pic").val()!=""){
		...
		if(document.getElementById('company_pic').getAttribute('src') == "image/file_demo.png"){
			...
			$.ajax({
						...
						data:{
							...
							user_role_id: 3,
							parent_user_id: $("#add_distributer").val(),
							created_by_id: 1
						},
						success: function(data){
							if(data.error){
		...
		}else if($("#add_distributer").val() ==""){
			$.notify({
				   message: 'Please select distributer name.'
				   },{
					type: 'danger',
					z_index: 9999,
					delay: 1000,
...
function editUserDetails(userid){
	...
	if($("#edit_first_name").val()!="" && $("#edit_last_name").val()!="" && $("#edit_email").val()!="" && $("#edit_mobile").val()!="" && $("#edit_city").val()!="" && $("#edit_state").val()!="" && $("#edit_pincode").val()!="" && $("#edit_distributer").val()!=""){
		...
		if(document.getElementById('editcompany_pic1').getAttribute('src') == "image/file_demo.png"){
			...
			$.ajax({
						url: serverURL + 'update_dealer_profile_details_by_id_admin',
						dataType: 'json',
						type: 'post',
						data:{
							...
							company_logo: company_pic,
							parent_user_id: $("#edit_distributer").val(),
							modify_by_id: userid,
							id: userid 
						},
						success: function(data){
		...
		}else if($("#edit_distributor").val() ==""){
			$.notify({
				   message: 'Please select distributer.'
				   },{
					type: 'danger',
					z_index: 9999,
					delay: 1000,
			...
			success: function(data){
				if(data.error){
					...
					$.map(data.user_list,function(item){
						...
						$("#add_distributer").append('<option value="'+item.id+'">'+item.first_name1+' '+item.last_name+'</option>');
					});
				}
			},
			complete: function(){
